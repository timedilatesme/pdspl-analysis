...
Updated to both generate and infer using a unified collective scatter (sigma_beta_E).
"""
...
def draw_lens_from_given_zs(z_lens, z1, z2, 
                            lambda_mst_mean, gamma_pl_mean, 
                            sigma_beta_E_true, sigma_meas_rel, 
                            cosmo, down_sampling=1, with_noise=True,
                            redshift_error_rel=0.0):
    """
    Generates mock data utilizing a unified structural scatter.
    """
    ...
    if redshift_error_rel > 0:
        ...
        dbeta_dzl, dbeta_dzs1, dbeta_dzs2, sigma_photoz_fid_sq = 0.0, 0.0, 0.0, 0.0
    
    # 5. True Population Variance combines the unified scatter and photo-z errors
    sigma_pop_true_sq = (sigma_beta_E_true * mu_model)**2 + sigma_photoz_fid_sq
    ...
    return {
        ...
        "sigma_beta_dspl": np.sqrt(sigma_meas_sq), # Pure Measurement Error
...
ALL_PARAM_NAMES = [
    'h0', 'om', 'w0', 'wa', 
    'lambda_int', 'gamma_pl', 'sigma_beta_E'
]
...
class DSPLLikelihood:
    ...
    def log_likelihood(self, theta):
        ...
        # 1. Structural Variance -> Unified Collective Scatter
        sigma_pop_sq = 0
        if 'sigma_beta_E' in self.sampled_params:
            sigma_pop_sq = (p['sigma_beta_E'] * model_mu)**2
        elif 'sigma_beta_E' in self.fixed_params:
            sigma_pop_sq = (self.fixed_params['sigma_beta_E'] * model_mu)**2
            
        # 2. Photo-Z Variance Propagation
        sigma_photoz_sq = 0
...
def run_dspl_inference(kwargs_dspl_list, down_sampling=1, n_walkers=32, n_steps=1000, n_burn=200, 
    ...
    default_start = {
        ...
        'lambda_int': 1.0, 'gamma_pl': 2.0, 'sigma_beta_E': 0.06
    }
