...
Updated to infer heteroscedastic (dissimilarity-dependent) intrinsic beta scatter.
"""
...
def draw_lens_from_given_zs(z_lens, z1, z2, 
                            ...
                            dissimilarity=0.0, # <-- ADDED to store the metric
    ...
    return {
        ...
        "dissimilarity": dissimilarity,               # <-- Passed to dictionary
...
ALL_PARAM_NAMES = [
    ...
    'beta_c0', 'beta_c1', 'beta_c2' # <-- Polynomial coefficients for intrinsic scatter
]
...
class DSPLLikelihood:
    def __init__(self, kwargs_likelihood_list, sampled_params, fixed_params, down_sampling=1, priors=None):
        ...
        self.dissimilarity = np.array([d.get('dissimilarity', 0.0) for d in kwargs_likelihood_list]).flatten() # <-- Extract dissimilarity
        ...
        self.priors = {
            ...
            'beta_c0': ('uniform', -1.0, 1.0), # Intercept
            'beta_c1': ('uniform', -5.0, 5.0), # Linear
            'beta_c2': ('uniform', -5.0, 5.0)  # Quadratic
        }
    ...
    def log_likelihood(self, theta):
        ...
        # 1. Evaluate all Three Scatter Components (Heteroscedastic Model)
        dy_dlam, dy_dgam = self.get_derivatives(beta_geo, p['lambda_int'], p['gamma_pl'], model_mu)
        
        # Build the intrinsic scatter per-lens using the polynomial coefficients
        ...
        # We can pull directly from 'p' which safely holds both sampled and fixed params
