...
Updated to infer structural scatters (gamma, lambda) AND intrinsic beta scatter simultaneously.
"""
...
def draw_lens_from_given_zs(z_lens, z1, z2, 
                            ...
                            sigma_beta_intrinsic, sigma_meas_rel, # Separated intrinsic & meas
    ...
    # 5. Calculate absolute variances combining ALL scatters
...
class DSPLLikelihood:
    def __init__(self, kwargs_likelihood_list, sampled_params, fixed_params, down_sampling=1, priors=None):
        ...
        self.priors = {
            ...
            'sigma_beta_intrinsic': ('uniform', 0.0, 0.5) # <-- Prior for intrinsic
        }
    ...
    def log_likelihood(self, theta):
        ...
        # 1. Evaluate all Three Scatter Components
        ...
        if 'sigma_beta_intrinsic' in self.sampled_params:
            sigma_pop_sq += (p['sigma_beta_intrinsic'] * model_mu)**2
...
def run_dspl_inference(kwargs_dspl_list, down_sampling=1, n_walkers=32, n_steps=1000, n_burn=200, 
    ...
    latex_labels = {
        ...
        'sigma_beta_intrinsic': r"$\sigma_{\beta,\rm int}$"
    }
